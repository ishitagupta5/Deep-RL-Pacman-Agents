...
def make_get_data_and_monitor_perceptron():
    ...
    if use_graphics:
        ...
        text = ax.text(0.03, 0.97, "", transform=ax.trans_axes, va="top")
    ...
    def monitor(perceptron, epoch, point, log):
        ...
        if log:
            print("epoch {:,} point {:,}/{:,} weights {}".format(epoch, point, points, w))
...
def get_data_and_monitor_digit_classification(model):
    ...
    if use_graphics:
        ...
        for i in reversed(range(10)):
            ...
            ax[i].set_yticks([])
            ax[i].text(-0.03, 0.5, i, transform=ax[i].trans_axes, va="center")
        ...
        status = ax[0].text(0.5, 1.5, "", transform=ax[0].trans_axes, ha="center", va="bottom")
        plt.show(block=False)
    ...
    def monitor(epoch, log):
        ...
        if log:
            print("epoch {:.2f}/{:.2f} validation-accuracy {:.2%}".format(epoch, epochs, dev_accuracy))

        if use_graphics:
            status.set_text("epoch: {:.2f}/{:.2f}, validation-accuracy: {:.2%}".format(epoch, epochs, dev_accuracy))
...
alphabet above have been substituted with ASCII symbols.""".strip())
    ...
    def monitor(iteration):
        ...
        for idx in spotlight_idxs:
            ...
            print(word_template.format(word, language_names[all_correct[idx]], all_predicted_probs[idx, all_correct[idx]],
                "" if correct else predicted_template.format(language_names[all_predicted[idx]]), probs=all_predicted_probs[idx,:],
            ))
        print("")
...
def get_data_and_monitor_online_rl(model, target_model, agent, env):
    ...
    # env = gridworld.GridworldEnvironment(gridworld.get_cliff_grid())
    ...
    for episode in range(n_episode):
        ...
        while not done:
            ...
            s2, r = env.do_action(a)
            steps += 1

            done = env.gridworld.is_terminal(s2) # deleted info
        ...
        if len(rewards) == rewards.maxlen:
            ...
            if np.mean(rewards) >= reward_threshold:
                print("Completed in {} episodes with mean reward {}".format(episode + 1, np.mean(rewards)))
