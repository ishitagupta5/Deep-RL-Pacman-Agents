...
#        GameState.explored.add(self)
    ...
    def get_ghost_position(self, agent_index):
        if agent_index == 0:
            raise Exception("Pacman's index passed to getghost_position")
...
class ClassicGameRules:
    ...
    def new_game(self, layout, horizon, pacman_agent, ghost_agents, display, quiet=False, catch_exceptions=False):
        agents = [pacman_agent] + ghost_agents[:layout.get_num_ghosts()]
        init_state = GameState()
        init_state.initialize(layout, len(ghost_agents))
        game = Game(agents, horizon, display, self, catch_exceptions=catch_exceptions)
...
def read_command(argv):
    ...
    parser.add_option('-m', dest='max_horizon', type='int',
    ...
    args['horizon'] = options.max_horizon
    ...
    if options.pacman == "PacmanDeepQAgent":
        print("options.agent_args", options.agent_args)
        layout_str = "layout_input={}".format(options.layout)
        if options.agent_args:
            options.agent_args += layout_str
        else:
            options.agent_args = layout_str
...
def run_games(layout, horizon, pacman, ghosts, display, num_games, record, num_training=0, catch_exceptions=False, timeout=30):
    ...
    for i in range(num_games):
        # if i % 10 == 0:
        #     print("num_games played: [{}/{}]".format(i, num_games))
        ...
        if be_quiet:
            ...
            rules.quiet = False
        game = rules.new_game(layout, horizon, pacman, ghosts, game_display, be_quiet, catch_exceptions)
        game.run()
        if not be_quiet:
            games.append(game)
