...
import test_classes
#from test_classes import TestCase
import random, math, traceback, sys, os
import layout, text_display, graphics_display, pacman, gridworld
import time
...
from pacman import run_games, load_agent
pp = PrettyPrinter()
VERBOSE = False
...
class QLearningTest(test_classes.TestCase):
    ...
    def run_agent(self, module_dict, num_experiences):
        ...
        # agent = module_dict['qlearningAgents'].LearnedQAgent(self.grid)
        states = [state for state in self.grid.get_states() if len(self.grid.get_possible_actions(state)) > 0]
        states.sort()
        last_experience = None
        for i in range(num_experiences):
        ...
        for state in states:
            ...
            for action in actions:
                ...
                if action in possible_actions:
                    ...
                    q_values[action][state] = None
        # print(agent.get_q_value([0,0], "exit"))
        values_pretty = self.pretty_values(values)
        policy_pretty = self.pretty_policy(policy)
        q_values_pretty = {}
        for action in actions:
