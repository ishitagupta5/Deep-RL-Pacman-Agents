...
class DoubledNumberPassedQuestion(Question):
    """Grade is the number of test cases passed."""

    def execute(self, grades):
        grades.add_points([f(grades) for _, f in self.test_cases].count(True)*2) # doubling the points for each passed test


class TripledNumberPassedQuestion(Question):
    """Grade is the number of test cases passed."""

    def execute(self, grades):
        grades.add_points([f(grades) for _, f in self.test_cases].count(True)*3) # tripling the points for each passed test
